...
- **Implemented:** Supports saving as `.svg` and `.png` by invoking the Mermaid CLI (`mmdc`) installed via `@mermaid-js/mermaid-cli`.
    - Uses `child_process.execFile` to run `mmdc`.
    - Includes error handling for `mmdc` process (not found, rendering errors).
    - Uses a temporary file for input to `mmdc`.
- A "Save As..." or "Save Diagram" button (`vscode.Command`) is added to chat responses *and* the webview for `/simpleUML`, `/relationUML`, and `/sequence` in `src/simple.ts`, triggering the `diagram.saveAs` command with the generated Mermaid syntax.

## Dependencies

- `mermaid` npm package.
- `@mermaid-js/mermaid-cli` npm package (dev dependency).
- VS Code Webview API.
...
*   **Webview Interaction:**
    ...
    *   "Export Diagram As..." button/dropdown in the webview panel, triggering a `diagram.saveAs` command.
*   **Diagram Export:**
    *   `diagram.saveAs` command allows exporting diagrams as Mermaid syntax (`.mmd`), Markdown (`.md`), SVG (`.svg`), or PNG (`.png`).
    *   Uses `@mermaid-js/mermaid-cli` for SVG/PNG generation.
...
*   **Webview HTML:** Generated by `getMermaidWebviewHtml` in `src/views/mermaid-webview-template.ts`.
*   **Mermaid CLI Integration:** Implemented within the `diagram.saveAs` command handler in `src/extension.ts`.
...
*   `/simpleUML` and `/relationUML` include prompts for the LLM to automatically detect appropriate diagram types based on code analysis.
*   Mermaid CLI (`mmdc`) is expected to be available (attempts to find in workspace `node_modules/.bin` first, then checks some global paths).
*   JSDOM/DOMPurify are used server-side for syntax validation (`validateMermaidSyntax`). This adds dependencies.
*   Webview communication (`postMessage`, `onDidReceiveMessage`) handles theme changes and export requests.
*   The "Save As..." or "Export Diagram" button (`vscode.Command`) is added to chat responses *and* the webview for `/simpleUML`, `/relationUML`, and `/sequence`, triggering the `diagram.saveAs` command with the generated Mermaid syntax.
